-- SUM(`Quantity`) ==> The Sum of Quantities will adjust the aggregation of buying and returning a product
-- count(distinct `StockCode`) ==> The count of unique product brought by the customer
-- sum(UnitPrice * Quantity) ==> The Total amount spent by the customer is UnitPrice * Quantity
